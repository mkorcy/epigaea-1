...
RSpec.describe Tufts::InputSanitizer do
  let(:params) do
    eval(File.read("#{fixture_path}/files/office_document_bug.rb")) # rubocop:disable Security/Eval
  end
  ...
  it "leaves tinymce formatting in place" do
    tiny_mce_output = File.read("#{fixture_path}/files/tinymce_output.txt")
